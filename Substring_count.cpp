...
void solve(ll cs){
    ...
    // p is the array containing indices of the suffix array
    // Problem : Find the number of occurances of a given string as a substring of another string.
    // Solution: Upper bound minus lower boung of the given pattern in the suffix array.
    ...
    while(tt--){
        ...
        while(left <= right){
            mid = (left + right) / 2;
            string temp = s.substr(p[mid], str.size());
            if(temp < str){
                left = mid + 1;
            }else{
                right = mid - 1;
