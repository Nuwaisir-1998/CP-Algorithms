    ...
    for(auto v : adj[u]){
        ...
        if(!vis[v]){
            ...
            if(low[v] >= tin[u] and par != -1){
                articulation_points.insert(u);          // A point may insert multiple times. So, set
            }
            if(low[v] > tin[u]){
...
void find_articulation_points_and_bridges(ll n){
    timer = 0;
...
void solve(ll cs){
    ...
    cout << (ll)articulation_points.size() << endl;
}
